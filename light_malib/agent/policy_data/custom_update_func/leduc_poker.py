...
def update_func(policy_data_manager, eval_results, **kwargs):
    ...
    Logger.info("policy_data: {}".format(
        policy_data_manager.format_matrices_data(["payoff", "score", "win", "lose", "reward"])))
    ...
    ray.get(monitor.add_array.remote("PSRO/Nash_Equilibrium/Payoff Table",payoff_matrix,
                                     xpid=pid, ypid=pid, global_step = payoff_matrix.shape[0], color='bwr'
                                     ))
    ...
    missing_policy_ids = [policy_data_manager.agents[training_agent_id].policy_ids[missing_index] for missing_index in
                          missing_indices]
    missing_policy_descs = ray.get(
        [policy_data_manager.policy_server.pull.remote(policy_data_manager.id, training_agent_id, policy_id)
    ...
    for agent_id in policy_data_manager.agents.keys():
        ...
        agent_id2policy_indices[agent_id] = np.array(
            [policy_data_manager.agents[agent_id].policy_id2idx[policy_id] for policy_id in population.policy_ids])
    ...
    monitor = get_actor(policy_data_manager.id, "Monitor")
    ray.get(monitor.add_scalar.remote("PSRO/Nash_Equilibrium/Exploitability", nash_eq_exploitability,
                                      len(policy_data_manager.nash_eq_exploitabilities) - 1))
...
def convert_to_official_tabular_policy(policy):
    ...
    ret = policy.compute_action(**{EpisodeKey.CUR_OBS: observations, EpisodeKey.ACTION_MASK: action_masks},
                                explore=False)
...
def get_nash_equilibrium_policy(policies, probs):
    ...
    policy = aggregate_policies(game, total_policies=[policies, policies],
                                probabilities_of_playing_policies=[probs, probs])
    return policy